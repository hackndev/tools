...
int gpio_get(void *map_base, int gpio) {
	volatile u32 *reg = (u32*)((u32)map_base + regoff(gpio));
	return (*reg >> (gpio&31)) & 1;
}

void gpio_set(void *map_base, int gpio, int val) {
	volatile u32 *reg = (u32*)((u32)map_base + (val ? GPSR : GPCR) + regoff(gpio));
	*reg = 1 << (gpio & 31);
}
...
int main(int argc, char **argv) {
    ...
    /* Map one page */
    map_base = mmap(0, MAP_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0x40E00000);
